Pro Grp_l2v7_pca_file_gen_1a_modInterp, File1, File2, Title=Title

  ; File 1 : hrs data for each module in the SCC folder. EvtQuicklook file *hkd.txt
  ; file 2 : Grp l2v7 flt investigate file. this is from pc file 1
...
;  ReadCol, File2, GpsTime2, Alt2, Dep, DepErr, Zen, ZenErr, Azi, Count , CountErr, Format='D,D,D,D,D,D,D,D,D'
  ReadCol, File2, SwpNo, GpsTime2, Alt2, Dep, Zen,Azi, Format='I,D,D,D,D,D'
  ...
  ; Only get the average mod temp. 
  AvgTemp = DblArr(N_Elements(Res31))
  NoVal =N_Elements(Res31)
  For i = 0,N_Elements(GPSTime2)-1 Do Begin
      AvgVal = Res0[i] + Res2[i]+ Res3[i] +Res4[i] +Res6[i] +Res7[i] +$
               Res9[i] +Res10[i]+ REs11[i]+Res12[i]+Res13[i]+Res14[i]+$
               Res17[i]+Res18[i]+ REs19[i]+REs20[i]+Res21[i]+Res22[i]+$
               Res24[i]+Res25[i]+ Res27[i]+Res28[i]+Res29[i]+Res31[i]
      AvgTemp[i]= AvgVal/NoVal
  EndFor
  
  
  Openw, Lun3, Cur+'/'+Title+'pca_1a_modtemp.txt',/Get_Lun
  Printf, lun3, 'GpsTime      AvgModTemp'
 For i = 0,N_Elements(GPSTime2)-1 Do Begin
      Printf, LUn3, GPSTime2[i], AvgTemp[i]
  EndFor
