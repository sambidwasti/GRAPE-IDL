pro GRP_evt_quicklook_V1, fsearch_string, nfiles=nfiles, iverbose=iverbose, title=title
  ...
  ;
  ;-- Grabbing information from file name.
  ;
  ...
  ;
  ; The module position numbers (module ids) for the modules installed for the 2014 flight.
  ;
  ...
  if keyword_set(title) ne 1 then begin
    ...
    cgPS_Open, file=title+'_evt_quicklook.ps'
    cgDisplay, 550, 800
    cgLoadCT, 13
  ...
  for ifile = 0, nfiles-1 do begin
    ...
    ;evdata = evdata[1:nevts[ifile]-1]
    ;nevts[ifile] = nevts[ifile] - 1

    filedt[ifile] = evdata[nevts[ifile]-1].gpstim - evdata[0].gpstim
    ;
    ;
    print
    print
  ...
  for ifile  = 0, nfiles-1 do begin
    ...
    if cts ne 0 then h_tot[*,ifile]   = float(histogram(value_locate(tlower, evtall[index].gpstim))) / float(tbin)

    ;
    index = where(*time_filter[ifile]and pc_filter, cts)
    if cts ne 0 then h_pc[*,ifile]   = float(histogram(value_locate(tlower, evtall[index].gpstim))) / float(tbin)
    ;
    index = where(*time_filter[ifile]and cc_filter, cts)
    if cts ne 0 then h_cc[*,ifile]   = float(histogram(value_locate(tlower, evtall[index].gpstim))) / float(tbin)
    ;
    index = where(*time_filter[ifile]and c_filter, cts)
    if cts ne 0 then h_c[*,ifile]   = float(histogram(value_locate(tlower, evtall[index].gpstim))) / float(tbin)
    ;
  ...
  cgPlot, filedt, psym=10, $
    charsize = 1.0, $
    xtickformat='(I2)', ytickformat='(I4)', $
    xtitle = 'File Number', ytitle = 'Elapsed Time (secs)', $
    title = 'Elapsed Time per Input EVT File', $
    Position= cgLayout([1,2,2], xgap=0, ygap=12, oxmargin = [5,5], oymargin = [10,5]), /noerase
  ...
  for i = 0, 31 do begin
    ...
    for ifile  = 0, nfiles-1 do begin
      ...
      if cts ne 0 then h_tot[*,ifile]   = float(histogram(value_locate(tlower, evtall[index].gpstim), min=0, max=9)) / float(tbin)
      ;
      index = where(*time_filter[ifile] and *mod_filter[i] and pc_filter, cts)
      if cts ne 0 then h_pc[*,ifile]    = float(histogram(value_locate(tlower, evtall[index].gpstim), min=0, max=9)) / float(tbin)
      ;

      index = where(*time_filter[ifile] and *mod_filter[i] and cc_filter, cts)
      if cts ne 0 then h_cc[*,ifile]    = float(histogram(value_locate(tlower, evtall[index].gpstim), min=0, max=9)) / float(tbin)
      ;
      index = where(*time_filter[ifile] and *mod_filter[i] and c_filter, cts)
      if cts ne 0 then h_c[*,ifile]     = float(histogram(value_locate(tlower, evtall[index].gpstim), min=0, max=9)) / float(tbin)
      ;
      h_times[*,ifile] = fevtime[ifile] + (findgen(bps)+0.5)*tbin
      h_jd[*,ifile]    = jdmin + (h_times[*,ifile] - gpsmin) / 86400.0
      ...
      for imod = fmod, fmod+7 do begin
        ...
        if (mcounts[imod] ge 1) and (firstpltflg eq 0) then begin
          ...
          plot, xtimes, hkdall[where(hkdall.modid eq imod)].hv, $
            psym=8, color = cgColor(pcolors[0]), symsize = 1.0, $
            charsize = 0.7, yrange = [min(hkdall.temp),max(hkdall.temp)], $
            ytickformat='(I6)', $
            XTICKFORMAT = ['LABEL_DATE', 'LABEL_DATE'], $
            XTICKUNITS = ['Time', 'Day'], $
        ...
        if (mcounts[imod] ge 1) and (firstpltflg eq 1) then begin
          oplot, xtimes, hkdall[where(hkdall.modid eq imod)].temp, psym=8, color = cgColor(pcolors[imod-fmod]), symsize = 1.0
        endif
