Pro Grp_L2v7_Flt_Investigate6, fsearch_String
  ;
...
Openw, lun9 , 'Flt_plan_Inves6.txt', /Get_Lun
printf, lun9, ' EvtTime   Trun_Julian_Date  Latitude Longitute Atm_Altitude  Source PntZen  PntAzi Cor Livetime'
  For p = 0, nfiles-1 Do Begin ; open each file
    ...
    For i = 0, TotPkt-1 Do Begin
		...
		NewTime = Event_Data[i].Evtime
		If NewTime-Old_time GE T_Int then begin
			
			Tjd =Event_data[i].TJD
			Lat =Event_data[i].Lat
			Lon =Event_data[i].Lon
			Alt = Event_data[i].Alt 
			Src = Event_data[i].source
			ZEn = Event_data[i].pntzen
			azi = Event_data[i].pntazi
			clt = Event_data[i].correctlt
			rlt = Event_Data[i].Livetime
			Printf, lun9, NewTime, Tjd, lat, Lon, Alt, src, zen, azi,clt,rlt, format='(F10.2, 1X,F10.2,1X,F10.2,1X,F10.2,1X,F10.2,2X,I1, 1X,F10.2,1X,F10.2, 1X, F10.5, 1X, F10.5 )'
