Pro Flt_l1_Pol, fsearch_String, nfiles=nfiles, title=title, Ener=Ener, Type = Type
; *************************************************************************
; *           Create a Polarization and PC Energy Spectrum                *
; *************************************************************************
; * Version: 1.01                                                         *
; *                                                                       *
; * Purpose:  Read in the Level 1 Version 7 processed files and generate  *
; *           Polarization spectrum and PC energy spectrum Histograms     *
; *                                                                       * 
; *                                                                       *
; * References: Flt_L1_Read.pro                                           *
; *            [ Reads in Flight level 1 version 6 files ]                *
; *                                                                       *
; * Usage: Flt_l1_Pol, 'L1*.dat'                                          *
; *                                                                       *
; *                     ******KEYWORDS*******                             *
; *     Keyword Parameters::                                              *
...
; *                                                                       *
; *     Ener  = A flag which skips polarization measurements. Useful for  *
; *             Flood runs to just get a PC Energy Spectrums.             *    
; *                                                                       *
...
; *                                                                       *
; *     Outputs::                                                         *
; *          Creates a histogram text files for polarization and PC Energy*
; *          spectrum.                                                    *
; *                                                                       *
...
; *                                                                       *
; *         9/21/14  Sambid Wasti                                         *
; *                  Changed the way to read in the structure data        *
; *                  It quicker and less cluttered way.                   *
; *                  Reading in a structure directly                      *
; *                                                                       *
...
False=0
    ...
    Energy_Max = Peak_Energy+Peak_Energy_Width
    Energy_Min = Peak_Energy-Peak_Energy_Width
...
;Needs to be updated below.
    ...
    Flag_Mod =   [3, 7, 7, 7, 11, 13, 14]        ; Pos No.
    ;These Mod = 104   106    107 108      121(7)   ; Respective Ser. No.
    Flag_Anodes =[34,25, 33, 41, 38, 49, 45]; Anodes in these modules. 
    ;
    ;
    ...
    Struc = { Version_Number    : 0B  ,$    ; 1 1
     ...
     For p = 0, nfiles-1 Do Begin ; open each file
         ...
         For i = 0, TotPkt-1 Do Begin
              ...
              ;-- Total Time.
              ;
              Temp_Time = Event_Data[i].Swept_Time ; For total time ran. ( We add after each file ran )
              
              ;
              ;Event Selection for The PC.---- Select 2 anodes and PC Event and exclude intermodule events. 
              ;
              If (Event_data[i].Event_Class NE 3) Or (Event_Data[i].No_Anodes NE 2) Or (Event_Data[i].Int_Mod_Flag EQ 1) Then goto, Jump_Packet
              
              
              ;
              ; -- Grab and Select the modules position that is currently being used
              ;
              ...
              If Type_Flag Eq True Then Begin
                      If(EventType(Pri_Anode,Sec_Anode) NE Type) Then Goto, Jump_Packet
              EndIF
              ...
              if Event_data[i].Cor_Live_Time LT 0 then begin
               print, event_Data[i]
                print,'what'
                stop
              endif
              ...
              ;
              ;  ---- Checking for the flag to skip the polarized stuffs or not
              ;
              IF Ener_Flag EQ 1 Then Goto, Jump_Packet  
              ...
              ;
              If (Tot_Energy LT Energy_Min) and (Tot_ENergy GT Energy_Max) Then Goto, Jump_Packet
              ...
              If (Temp_Value LT -1) or (Temp_Value GT 1) Then BEgin
                         ...
                         ;
                         Temp_Anode = Pri_Anode
                         Pri_Anode = Sec_Anode
                         Sec_Anode = Temp_Anode
                         ...
                         Temp_Value= 1 - (511.0*( (Tot_Energy/Sec_Energy)-1)/ Tot_Energy)
                         If (Temp_Value LT -1) or (Temp_Value GT 1) Then Goto, Jump_Packet ; Skip if compton angle still invalid after the swithc. 
              EndIF 
              
              ; --- All section completed now grabbing the Scattering angle
              ...
              ;---- Grab Co-ordinates ---
              ;
              Pri_Cord = Grp_AnodeXY(Pri_Anode)+Grp_ModuleXY(Mod_Pos)
              Sec_Cord = Grp_AnodeXY(Sec_Anode)+Grp_ModuleXY(Mod_Pos)
...
;------------------------
              ...
              ; The rotation angle should be the Offset angle - table angle           ;
              ...
              ;
              Rot_Angle_Measured = Offset_Angle - Event_Data[i].Rotation_Angle
...
;Print, Pri_Anode, Sec_Anode, Rot_Angle_Measured, Pres_Ves_Angle           
;----------------------------------------
    ...
    ; 
    ; --- Flag to skip if only PC Energy Spectrum Needed.
    If Ener_Flag EQ 1 Then Goto, Jump_Pol
    ...
    CgHistoplot,Main_Src_Arr, Binsize=5.0, /OUTLINE, XRANGE=[0,360], XSTYLE=1
    ...
    Openw, Lunger, Cur+'/'+Title+'_n'+strn(nfiles)+'_PolHist.txt', /Get_Lun
          ...
          Printf, Lunger, 'Energy range ='+Strn(Peak_energy-Peak_energy_Width)+' ~ '+Strn(Peak_energy+Peak_energy_Width)
    ...
    ;
    Openw, Lunger1, Cur+'/'+Title+'_n'+strn(nfiles)+'_PC_EnerHist.txt', /Get_Lun
          ...
          For i =0,31 Do Begin
                      ...
                      ; - live time stuffs -
                      ;
                      If Live_Time_Cnt[i] Eq 0.0D then AvgLT=0 Else AvgLT=(Double(Tot_Live_Time[i])/Double(Live_Time_Cnt[i]))
