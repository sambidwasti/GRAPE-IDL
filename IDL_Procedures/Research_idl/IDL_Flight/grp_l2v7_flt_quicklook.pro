Pro Grp_l2v7_Flt_quicklook, fsearch_String, nfiles=nfiles, title= title
  ;
...
Xmax = 101
  ...
  PC_T2_Events     = DblArr(102)
  PC_T3_Events     = DblArr(102)
  Sweep_Ar         = INDGEN(102)
 ...
 ;   print, PC_Counter
    ...
    print, Event_Data[1].sweepno
    Scaled_No_Events[Event_Data[1].sweepno] = Temp_PC_Cnt/(Max(Event_Data.Swtime)) ; we have (Avg_LT_Counter)* Avg_LT/Avg_LT_Counter * 720/ (Total Time * 720) for the sweep
    PC_T1_Events[Event_Data[1].sweepno]     = Temp_PC1_Cnt/(Max(Event_Data.Swtime))
    ...
    PC_T2_Events[Event_Data[1].sweepno] = PC_T2_Counter/(Average_LT*(Max(Event_Data.Swtime))) ; we have (Avg_LT_Counter)* Avg_LT/Avg_LT_Counter * 720/ (Total Time * 720) for the sweep
    PC_T3_Events[Event_Data[1].sweepno] = PC_T3_Counter/(Average_LT*(Max(Event_Data.Swtime))) ; we have (Avg_LT_Counter)* Avg_LT/Avg_LT_Counter * 720/ (Total Time * 720) for the sweep


    ;--- Per Sweep Data Generated directly ---
    Swp_Alt_Ar[Event_Data[5].Sweepno]=Avg(Event_Data.Alt)
    Swp_Evt_Ar[Event_Data[5].Sweepno]= TotPkt
    ;Swp_Time_Ar=[Swp_Time_Ar, Avg(Event_Data.SwTime)+Event_Data[i-1].SwStart]
 
    Swp_SweepNo_Ar= [Swp_SweepNo_Ar, Event_Data[5].Sweepno]
...
;  PC_T1_Events     =  PC_T3_Events[where( PC_T3_Events  NE 0)];  ;  CgPS_Close
  ...
  cgPS_Open, Title+'_Level2_quicklook.ps', Font=1
  cgDisplay, 800, 800
  cgLoadCT, 13

  CgPlot, Swp_SweepNo, (Swp_Alt/(1000.0)), Xtitle ='Sweep Number',Title='Altitude',Ytitle='Altitude (kft)',$
  XRange=[Xmin,XMax],XStyle=1,PSYM=45,YRange = [ 110,135], $
    Position= cgLayout([1,2,1], xgap=0, ygap=12, oxmargin = [5,5], oymargin = [10,5]), /noerase

  CgPlot, Swp_SweepNo, (Swp_Evt_Ar/(10000)), Xtitle ='Sweep Number',Title=' Total no. of Events',YTitle= ' Counts (10^4)',$
    XRange=[Xmin,XMax],PSYM=45,XStyle=1,YRange= [28,37], YStyle=1, $
  ...
  CgPlot,Swp_SweepNo,Scaled_No_Events, Xtitle ='Sweep Number',Title=' PC Counts',YTitle=' Counts/ Sec',$
    XRange=[Xmin,XMax],XStyle=1, PSYM=45,YRange= [10,20], YStyle=1, $
    Position= cgLayout([1,2,1],xgap=0, ygap=12, oxmargin = [5,5], oymargin = [10,5]), /noerase
  CgText, !D.X_Size*0.78,!D.Y_Size*0.97, 'Pla = 6~200 kev',/Device,Charsize=1.0
  CgText, !D.X_Size*0.78,!D.Y_Size*0.95, 'Cal = 20~400 kev',/Device,Charsize=1.0
  CgText, !D.X_Size*0.78,!D.Y_Size*0.93, 'Tot = 80~200 kev',/Device,Charsize=1.0
  ...
  CgPlot, Swp_SweepNo, PC_T1_Events, Xtitle ='Sweep Number',Title=' Normalized PC Type 1',YTitle=' Counts/ Sec',$
  ...
  cgPS_Close
  Temp_Str = Title+'_Level2_quicklook.ps'
