Pro Grp_L2v7_Flt_Investigate4, fsearch_String, Title= Title, Type=Type, Class = Class, Sep = Sep, Bin=bin

  ; Grp Investigate 3 copied atm. 
  ; Grp investigate 3 has all different types atm.
  ; This is to generate an energy loss spectrum and plot for the Flight data. 
  ; There is an option for class but atm only interested in PC.
  ;  Goal is to Bin
  ; Create an energy loss text file. 
  ;  Create a plot.
  ; This does for all the sweeps together to generate a counts vs energy range. 
  ;
close, /all
 ...
 Class=3
  ...
  ptitle = 'Energy Loss Spectrum for Crab Observation (PC Events)'
...
;  EnerErr4= DblArr(1000)
  ...
  openw, lun1, 'Grp2014_Crab.txt',/GET_LUN
  ...
  CgPs_Open, Title+'_Flt_Gamma_Inves4.ps', Font =1, /LandScape
  cgLoadCT, 13
  !P.Font=1
  ...
  xtitle = 'Energy (keV)'
  ytitle= 'Counts / s / keV'
  Xerr = IntArr(N_Elements(Xval2))+Bin/2
  Er_wid =0.0
  Cgplot, Xval2,Flight2, PSYM=10, XRANGE=[Xmin,Xmax],Xstyle=1, /Xlog,/Ylog,  YRAnge=[Ymin,Ymax], YStyle=1,$;
    ...
    err_XHigh = Xerr, Err_Xlow=Xerr,xgridstyle=1, xticklen=1, yticklen=1, ygridstyle=1, xtickinterval=10


  CgPs_Close

  ;-- Create the PDF File
  Temp_Str = Cur+'/'+Title+'_Flt_Gamma_Inves4.ps'
