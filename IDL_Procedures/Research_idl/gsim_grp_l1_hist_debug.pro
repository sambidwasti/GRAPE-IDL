Pro GSim_Grp_l1_hist_debug, fsearch_String, title=title, Bsize=Bsize
  ;
  ...
  ; 
  ; This is to debug the direct cg-histogram rebin in sim procedure vs the way its done in plot_hist_mac
  ; The pseudo histogram.
  ;
  ...
  CgPs_Open, ftitle+'_Gsim_Grp_L1.ps', Font =1, /LandScape
  cgLoadCT, 13
  ...
  ;  ScatHist = CGHistogram(Scat_arr, Min=0, Max=360, Locations=Xarr, Binsize=1)
    ScatHist = Histogram(Scat_arr, Min=0, Max=359, Locations=Xarr, Binsize=1)
    ;    print, xarr
    help,xarr, scathist
;    help, xarr1, scathist1
   ...
   CgPlot, xarr,Main_ScatHIst, PSYM=10, Xrange=[0,360], title='Simulation'
    ...
    ScatHist1 = Histogram(Scat_arr, Min=0, Max=359, Locations=Xarr1, Binsize=BSize)
    CgPlot, xarr1,ScatHIst1, PSYM=10, Xrange=[0,360], title= 'Simulation -Cg Rebin Bsize='+Strn(BSize)



    N_Bins =  N_elements(ScatHist)/BSize
     Print, N_Bins, bsize, N_elements(ScatHist)
    Scathist2 = DblArr(N_bins)
   Temp_Count =0L
    For i = 0, N_Bins-1 do begin
        Temp_Val =0.0
        For j=0, BSize-1 Do Begin
          Temp_Val = ScatHist[Temp_Count]+Temp_Val
          Temp_Count++
          
        EndFor;j
        ScatHist2[i]=Temp_Val

      EndFor ; i
      xarr2 = Indgen(N_bins)*Bsize

     CgoPlot,Xarr2, ScatHist2,PSYM=10, Xrange=[0,360], color='Blue'
     cgErase
;    Temp_Err_Hist= Sqrt(ABS(Double(Temp_Hist)))
  ...
  CgPs_Close
  Temp_Str = Cur+'/'+ftitle+'_Gsim_Grp_L1.ps'
