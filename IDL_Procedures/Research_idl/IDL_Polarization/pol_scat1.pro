Pro Pol_Scat1, Input_Folder, Bin=Bin, NoEner=NoEner, EBIN=EBIN, OPT=OPT
...
False= 0
    ...
    IF Back_Index NE -1 Then Begin
            ...
            Scat_Hist = DblArr(N_Elements(Back_Scat_Hist1))
            Scaled_Back_Scat_Hist1 = DblArr(N_Elements(Back_Scat_Hist1))
;            Scat_Hist = Src_Scat_Hist1-Scaled_Back_Scat_Hist
            
; Dividing the Source with the Scaled Background
            
            For i =0, N_Elements(Scaled_Back_Scat_Hist)-1 Do Begin
                      Scat_Hist[i]=(Total(Scaled_Back_Scat_Hist)* Src_Scat_Hist1[i])/(Scaled_Back_Scat_Hist[i]*N_Bins)
                      Scaled_back_Scat_Hist1[i]=(Total(Scaled_Back_Scat_Hist)* Scaled_Back_Scat_Hist[i])/(Scaled_Back_Scat_Hist[i]*N_Bins)
            Endfor
       ...
       loadct, 13                           ; load color
            ...
            Device, /inches, xsize = 7.0, ysize = 9.0, xoffset = 0.75, yoffset = 1.0, font_size = 9, scale_factor = 1.0
                         Plot, X_Val, Src_Scat_Hist1, PSYM=10,Title='Polarized Events',CharSize=2,$
                               XRange=[0,360], XStyle=1,XTickInterval=45, $
                               YRange=[-100,Max(Src_Scat_Hist1)*1.2],/NODATA,$;YRange=[Ymin,YMax],$
                               XTitle='Angle', YTitle='Counts'
                        ...
                        ; Oplot, X_Val2*360/(2*!PI), Fitted_Funct, Color=CgColor('Red')   
                         ...
                         Oplot, X_Val, Scaled_Back_Scat_Hist1, PSYM=10,Color=CgColor('Green')
...
;                                   
       If Energy_Flag Eq True Then Begin
                 ...
                 loadct, 13                           ; load color
                      ...
                      Device, /inches, xsize = 7.0, ysize = 9.0, xoffset = 0.75, yoffset = 1.0, font_size = 9, scale_factor = 1.0
                                   Plot, X_Ener_Val, Ener_Hist, PSYM=10, Title= 'Energy Histogram',CharSize=2,$
                                       XTitle='KeV', YTitle='Counts', YRange=[-1000,YMax1], YStyle=1$
                                   ...
                                   OPlot,X_Ener_Val,Src_Ener_Hist1, PSYM=10, Color=CgColor('Blue')
