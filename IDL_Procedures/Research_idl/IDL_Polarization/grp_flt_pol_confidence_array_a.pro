Pro Grp_Flt_Pol_Confidence_array_a, infile

  ; mu 100 from crab is 0.41 =/- 0.01
  ;NOTE : HERE mu is fixed. for C1 to be fixed go to version without a.
  ; for a fixed mu, C2 is adjusted for a change in C1. 
 ...
 ; Par[1].limited(0) = 1
  ...
  ;Print, '----'
  ;=== Confidence plot ==
  ; The modulation factor is Fit[1]/Fit[0] so fixing this part
  ; mu = fit[1]/fit[0]
...
LevelsArray =delX_array + chisqr;+chi
  window,2
  CgContour, ChiSqArray , Deg_Arr, Pol_Array, xrange=[0,360],xstyle=1,Levels=LevelsArray, Xtitle= 'Polarization Angle', Ytitle ='Polarization Fraction',Title ='Confidence Plot of del Chi-Sq (Constant C2)',$
   C_annotation=['  68%  ','  95%  ','  99%  ']
  CgOplot,angle+90, Fit[1]/(0.41),PSYM=1, SymSize=5.0, Color='Red'
