Pro Flt_Plot_Hist_mac, Src_File, Back_File , BSize=BSize ,Indiv = Indiv, Title=Title, Type=Type, NFIT=NFIT, Max_X = Max_X , OPT=OPT
  ...
  ; *         6/4/19   Sambid Wasti
  ; *                  Updated graphics for macbook                         *
  ; *                  Need to generate polarized text file output
  ; *                                                                       *
  ...
  ; -- Bin Size --
  If Keyword_Set(BSize) EQ 0 Then BSize=5
  ...
  IF Indiv_Flag EQ False THen Title=Title+'_All'
  ...
  For i=0,N_Fit-1 Do BEgin
    ...
    For j = 0,31 Do Begin
      ...
      While Flag_Fitted EQ False Do BEgin
        ...
        Polyfill, [656,656,696,696], [360,412,412,360], Color=CgColor('Purple'),/DEVICE
        XYOUTS, 660, 385, 'DONE', /Device, charsize=1.2

        Polyfill, [606,606,648,648], [360,412,412,360], Color=CgColor('ORANGE'),/DEVICE
        XYOUTS, 607, 385, 'GAUSS', COLOR=CgColor('Black'),/Device,charsize=1.2

        Polyfill, [552,552,600, 600], [389,412,412,389], Color=CgColor('ORANGE'),/DEVICE
        XYOUTS, 556, 400, 'Flat', COLOR=CgColor('Black'),/Device

        Polyfill, [552,552,600, 600], [360,385,385,360], Color=CgColor('ORANGE'),/DEVICE
        XYOUTS, 556, 370, 'SINO', COLOR=CgColor('Black'),/Device

        Polyfill, [500,500,546, 546], [389,412,412,389], Color=CgColor('ORANGE'),/DEVICE
        XYOUTS, 512, 400, '2GAUSS', COLOR=CgColor('Black'),/Device

        Polyfill, [500,500,546, 546], [360,385,385,360], Color=CgColor('ORANGE'),/DEVICE
        XYOUTS, 509, 370, 'SINO^2', COLOR=CgColor('Black'),/Device
        ...
        IF( X_Value GE 656) AND ( Y_VALUE GE 360 ) Then BEgin
          Flag_Fitted = True
          Print, '-----------------Fitting DONE ---------------------'
          GOTO, Jump_Fitted
        ENDIF
        ...
        ;
        If( (X_Value GE 606) And (X_Value LE 648) AND (Y_Value GE 360) And (Y_Value LE 412) ) Then F=0 $
        Else If( (X_Value GE 552) And (X_Value LE 600) AND (Y_Value GE 389) And (Y_Value LE 412) ) Then F=1 $
        Else If ( (X_Value GE 552) And (X_Value LE 600) AND (Y_Value GE 360) And (Y_Value LE 385) ) Then F=2 $
        Else If ( (X_Value GE 500) And (X_Value LE 546) AND (Y_Value GE 389) And (Y_Value LE 412) ) Then F=3 $
        Else If ( (X_Value GE 500) And (X_Value LE 546) AND (Y_Value GE 360) And (Y_Value LE 385) ) Then F=4
        ...
        If F EQ 0 Then Fit_Name = 'GAUSSIAN' Else $        ;Working
          If F EQ 1 Then Fit_Name = 'Flat'Else $
        ...
        ;
        XYOUTS, 500, 375, Fit_Name, CharSize= 3, /DEVICE  ; Outputing what function we clicked to keep track of it.

        ;
        ;--- Module No. for Individual Module Plots--
        ;
        IF Indiv_Flag eq True Then XYOUTS, !D.X_Size*0.5,!D.Y_Size*0.85, 'MODULE '+strn(j), CharSize=1,/Device

        ;
        ;-- No Fits for these fits yet so jump to Sino-Plot
        If (F EQ 2) OR (F EQ 4) OR (F EQ 1) Then Goto, Sino_Plot
        ...
        ;                                        60~702
        ;                                   slope = diff(ener) / diff(pixel
        ;
       
;        Cursor, X_Value900, Y_Value900, /DOWN, /DEVICE
;        PRint,X_Value900, Y_Value900
;        Cursor, X_Value901, Y_Value901, /DOWN, /DEVICE
;        Print,X_Value901, Y_Value901
;        Cursor, X_Value902, Y_Value902, /DOWN, /DEVICE
;        Print,X_Value902, Y_Value902
;        Stop
        C_Slope = Float( (Max_X-XMIN)/Float(642))
        ...
        Case F OF
          0: Begin      ; This is the GAUSSIAN.
            ...
            Fit_Text1= String(Format= '("Centroid =" ,(F6.1,X)," +/- ",(F4.1) )',Fit[1], PCERROR[1])
     ...
     Fit_Text12 =''
          ...
          4: Begin ; Sinosudial squared.
            ...
            Fit_Text1= 'MOdulation factor = '+Fit_Text1
      ...
      Ymin = Min([Final_Src, Final_Histogram, Final_SclBack])-Max(Final_SclBack_Err)

      ;
      ; --- Fixing Binning Problem for plotting in the middle of the bin rather than beginning.
      ;
      Xfit2= Xfit1;+(BSize/2.0) ;SWs
  ...
  If (Indiv_Flag EQ False) Then Begin
    ...
    CgPlot,XVAL2, Final_Histogram, PSYM=10,CharSize=2,$
      XRange=[0,max_x], XStyle=1,XTickInterval=50, $
      YRange=[Ymin,Y_Max2],/NODATA,ystyle=1,$
      XTitle=Title_X  , YTitle='Counts'
    ...
    If Type EQ 2 Then Begin
      ...
      CgText,  !D.X_Size*0.70,!D.Y_Size*0.97, 'Modulation ('+CgGreek('mu')+')'+ Fit_Text11  , Color=CgColor('Black'),/DEVICE, CharSize =1.7
      ;
      ; --- Fitted Total Counts
      ;
...
;  PRint, Total(Final_Histogram), Total(Final_SclBack), Total(Final_Src)
  ...
  Openw, Lun, Title+'_PlotHist_mac.txt', /Get_lun
    For i = 0, N_Elements(Final_Histogram)-1 Do Begin
        Printf,lun,XVal[i], Final_histogram[i], Final_Histogram_Err[i]
    EndFor
  Free_Lun,Lun
  Stop
