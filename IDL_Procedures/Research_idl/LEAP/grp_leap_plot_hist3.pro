Pro Grp_leap_Plot_hist3, Src, Back,  title=title
...
;  If Ener eq 1 then title= title+'_Ener'
  CgPs_Open, title+'Ener.ps', Font =1, /LandScape
  cgLoadCT, 13

  CGPlot, X, Src_Cnt, PSYm=10, Color='Blue', Title=Title+TableAngle+'Energy',Ytitle=' Counts', XTitle='Energy',$
    XRANGE=[0,400], XTICKINTERVAL=50, /Err_Clip;,yrange=[0,10000]
  CgOplot, X, Scl_Back, PSYM=10, COlor='RED', /Err_Clip
  CGOplot, X, Main_Hist, PSYM=10, COlor='black', Err_Ylow=Main_Err, Err_yHigh=Main_Err, /Err_Clip
  ...
  CgPlot, X, Main_Hist, PSYM=10, Xrange=[0,400], Title=Title+TableAngle+'Ener',Ytitle=' Counts', XTitle='Energy',$
    XTICKINTERVAL=50, ERr_Ylow=Main_Err, Err_yHigh=Main_Err, /ERR_Clip
  CgPs_Close
  Temp_Str = Cur+'/'+title+'Ener.ps'
 ...
 ; If Ener eq 1 then Text_title = Text_title+'Ener.txt' Else Text_title=Text_title+'_leap_plot_Hist2.txt'
  ...
  Openw, lun, Text_Title+'_Ener.txt', /Get_lun
