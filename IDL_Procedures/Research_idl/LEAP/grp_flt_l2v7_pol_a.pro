Pro Grp_Flt_l2v7_Pol_a, fsearch_String, nfiles=nfiles, title=title, Type = Type
  ;****************************************************************
  ; This is a child of Flt_l2v7_Pol.
  ; This is similar to flt l2v7 pol b but has an additional filter of rotation table angle.
  ...
  ;
  ; We Also want to check the rates so adding a rates histogram.
  ; 
  ; Note : This is has to be updated for the histograms being created 
  ; 
  ; BugFix 1.0  : Sambid Wasti 07/24/19
  ;               The histogram function is giving wrong bins and sizes by defining Nbins. 
  ;               This has been corrected with using binsize instead of Nbins 
  ;               Note: this should translate to pol_b and Pol.
  ;         
  ;
  True = 1
  False= 0
  ...
  EMAX= 1000 ; Max value of energy for energy histogram.
  ; === Energy Cuts Ended ==
  ...
  title = title+'_Rot_'+strn(Rot_min)+'_'+Strn(Rot_max)
  ...
  For p = 0, nfiles-1 Do Begin ; open each file
    ...
    For i = 0, TotPkt-1 Do Begin
      ...
      ; SW June 4, 2019
      ; Changing this to scat angle
      ; adding rotation angle filter.
      ;Scatter_Angle = Double( Event_Data[i].PVAng)
      
       rt_ang = event_data[i].rtang
       if (rt_ang LT double(Rot_Min)) Or (rt_ang GT double(Rot_Max)) Then goto,Jump_Packet
       ;print, rt_ang
        Scatter_angle = Double(Event_Data[i].sctang)
      ...
      ;
      If Mod_Pos EQ 0 Then P0_Arr  = [P0_Arr,Scatter_angle]
      If Mod_Pos EQ 2 Then P2_Arr  = [P2_Arr,Scatter_angle]
      If Mod_Pos EQ 3 Then P3_Arr  = [P3_Arr,Scatter_angle]
      If Mod_Pos EQ 4 Then P4_Arr  = [P4_Arr,Scatter_angle]
      If Mod_Pos EQ 6 Then P6_Arr  = [P6_Arr,Scatter_angle]
      If Mod_Pos EQ 7 Then P7_Arr  = [P7_Arr,Scatter_angle]
      If Mod_Pos EQ 9 Then P9_Arr  = [P9_Arr,Scatter_angle]
      If Mod_Pos EQ 10 Then P10_Arr = [P10_Arr,Scatter_angle]

      If Mod_Pos EQ 11 Then P11_Arr = [P11_Arr,Scatter_angle]
      If Mod_Pos EQ 12 Then P12_Arr = [P12_Arr,Scatter_angle]
      If Mod_Pos EQ 13 Then P13_Arr = [P13_Arr,Scatter_angle]
      If Mod_Pos EQ 14 Then P14_Arr = [P14_Arr,Scatter_angle]

      If Mod_Pos EQ 17 Then P17_Arr = [P17_Arr,Scatter_angle]
      If Mod_Pos EQ 18 Then P18_Arr = [P18_Arr,Scatter_angle]
      If Mod_Pos EQ 19 Then P19_Arr = [P19_Arr,Scatter_angle]
      If Mod_Pos EQ 20 Then P20_Arr = [P20_Arr,Scatter_angle]

      If Mod_Pos EQ 21 Then P21_Arr = [P21_Arr,Scatter_angle]
      If Mod_Pos EQ 22 Then P22_Arr = [P22_Arr,Scatter_angle]
      If Mod_Pos EQ 24 Then P24_Arr = [P24_Arr,Scatter_angle]
      If Mod_Pos EQ 25 Then P25_Arr = [P25_Arr,Scatter_angle]
      If Mod_Pos EQ 27 Then P27_Arr = [P27_Arr,Scatter_angle]
      If Mod_Pos EQ 28 Then P28_Arr = [P28_Arr,Scatter_angle]
      If Mod_Pos EQ 29 Then P29_Arr = [P29_Arr,Scatter_angle]
      If Mod_Pos EQ 31 Then P31_Arr = [P31_Arr,Scatter_angle]
  ...
  PSct_arr = CGHistogram(  Main_Src_Arr, Min=0, Max=360, Locations=Xarr, NBins=360)
  
   CgPs_Open, title+'_Grp_Pol_a.ps', Font =1, /LandScape
  cgLoadCT, 13
    CgPLot, xarr,PSCt_arr, PSYM=10, XRANGE=[0,360], Title=title
  CgPs_Close
  Temp_Str = Cur+'/'+title+'_Grp_Pol_a.ps'
  ...
  ;
  M0Arr = Histogram(H0_Arr, Binsize=1, Max = 359.0, Min=0.0, locations=xarr1)
  M2Arr = Histogram(H2_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M3Arr = Histogram(H3_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M4Arr = Histogram(H4_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M6Arr = Histogram(H6_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M7Arr = Histogram(H7_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M9Arr = Histogram(H9_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M10Arr = Histogram(H10_Arr, Binsize=1, Max = 359.0, Min=0.0)

  M11Arr = Histogram(H11_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M12Arr = Histogram(H12_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M13Arr = Histogram(H13_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M14Arr = Histogram(H14_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M17Arr = Histogram(H17_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M18Arr = Histogram(H18_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M19Arr = Histogram(H19_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M20Arr = Histogram(H20_Arr, Binsize=1, Max = 359.0, Min=0.0)

  M21Arr = Histogram(H21_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M22Arr = Histogram(H22_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M24Arr = Histogram(H24_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M25Arr = Histogram(H25_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M27Arr = Histogram(H27_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M28Arr = Histogram(H28_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M29Arr = Histogram(H29_Arr, Binsize=1, Max = 359.0, Min=0.0)
  M31Arr = Histogram(H31_Arr, Binsize=1, Max = 359.0, Min=0.0)
  ...
  Openw, Lunger, Cur+'/'+Title+'_n'+strn(nfiles)+'_Pola_PolHist.txt', /Get_Lun
  ...
  ;
  Emax1 = Emax-1
  M0EArr = Histogram(H0_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M2EArr = Histogram(H2_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M3EArr = Histogram(H3_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M4EArr = Histogram(H4_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M6EArr = Histogram(H6_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M7EArr = Histogram(H7_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M9EArr = Histogram(H9_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M10EArr = Histogram(H10_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)

  M11EArr = Histogram(H11_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M12EArr = Histogram(H12_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M13EArr = Histogram(H13_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M14EArr = Histogram(H14_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M17EArr = Histogram(H17_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M18EArr = Histogram(H18_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M19EArr = Histogram(H19_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M20EArr = Histogram(H20_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)

  M21EArr = Histogram(H21_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M22EArr = Histogram(H22_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M24EArr = Histogram(H24_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M25EArr = Histogram(H25_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M27EArr = Histogram(H27_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M28EArr = Histogram(H28_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M29EArr = Histogram(H29_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
  M31EArr = Histogram(H31_EArr, Binsize=1.0, MAX=EMAX1, MIN=0.0)
 ...
 ; Help, Scat_EArray, M0EArr
  ...
  ;
  Openw, Lunger1, Cur+'/'+Title+'_n'+strn(nfiles)+'_Pola_EnerHist.txt', /Get_Lun
  ...
  CGPlot, Rates[1:N_Elements(Rates)-1],PSYM=10, title=title, ytitle='Counts(Events) for a sweep'
  Free_Lun, Lunger1
End