Pro GSim_GRP_Investigate, fsearch_String, title=title, nfiles= nfiles, PSDeff=PSDeff, Per=Per
  ;
  ...
  if keyword_set(Psdeff) ne 1 then Per = 100
    Crs_talk = 'Inv_Ct0_7a_'+strn(per)+'_'+ STRN(PSDEff)
    print, 'Crs_Talk % =', Per
    print, 'file=',Crs_talk
  ;
  ;=== PSD Efficiency ===
  ...
  for p =0, nfiles-1 Do Begin
    ...
    For i =0L, nevents-1 do Begin
      ...
      For k = 0,NoAnodes-1 Do Begin
        ...
        ;
        Temp_Arr = Grp_CT_Al_1(New_AnID[k],New_AnNrg[k],Per=Per)
