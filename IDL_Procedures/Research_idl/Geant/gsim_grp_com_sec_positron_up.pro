Pro GSim_GRP_com_sec_positron_up, fsearch_String, title=title, bin=bin, Type=Type, foldername=foldername, N=N, Class = Class, Sep = Sep

  ; Secondary positron up
  ; This follows similar steps like the electron/positron up
  ; Note: It relies on the fact that the naming of the files (a,b,c..) would
  ;       sort the folders accordingly (appropriately) for the normalization
  ; TThis reads in the *lvl1.dat files. 
  ; Then this needs to apply the software threshold
  ; Then only generate the files.
  ; This is to combine the numerous proton files and combine them.
  ; Since there is a 1/cos(theta) dependence until 60 deg
  ; and its 2 till 90 deg.
  ; The degree bin defines the number of files.
  ; The N has to be double checked to make sure its the same number as the runs.
...
;  Print, 'P', Pla_Min, Pla_Max
  ...
  ;
  ; == Scaling Factors ==
  ; There are 7 files, file 1 is proton, then electron then positron.
  ; taking the mid values. Remember the last one is *2
  ...
  mev_const =56.0694+36.6638+0.0224834
 ...
 ;   print, p, ' ',infile
  ...
  openw, lun1,foldername1+title1+'_'+Evt_Text+'_l1_SecUpPosi_com.txt',/Get_Lun
