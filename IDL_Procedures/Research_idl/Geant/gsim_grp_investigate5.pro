Pro GSim_GRP_Investigate5, fsearch_String, title=title, nfiles= nfiles, PSDeff=PSDeff
  ;
  ;   Used for Analysis 2.1
  ;--Read the file created by the GSIM File from Read CSV ---
  ;     NB: * marks the further updates below.
  ;
  ;   Include the Cross-Talk Algorithm.
  ;   Correct the Energy with the energy broadening.
  ;             -- Eventually include this in the first processing file --
  ;   Apply energy-threshold cuts.
  ;   May 4th, 2016
  ;             -- Modifying the algorithm to get the output as the hardware classification.
  ;   May 12th, 2016
  ;             -- Adding ability to do multiple files at once.
  ;   May 18, 2016 S.Wasti
  ;       -- Adding 4 new arrays to keep track of the event classes reclassifications
  ;   May 24, 2016 S. Wasti
  ;       -- In order to further match the hardware triggers, we are adding in one intermediatary process to
  ;       recreate the hardware output with hardware triggers and then apply the software threshold. It is
  ;       reading in the hardware classified
  ;   June 1, 2016, S. Wasti
  ;       -- We are including efficiency and adding in the updated Cross-talk algorithm.
  ;       -- For this, we would like to keep track of the cross-talk via the adjacent types.
  ;       -- Therefore this has been updated in the Grp_Ct_Al_2a onwards (*1)
  ;       -- and updated in here.
  ;   June 16, 2016 S.Wasti
  ;       -- Added the decimation factor to decimate the C and CC events as in the calibration.
  ;   Sep 8, 2016 S. Wasti
  ;       -- Some issues with reproducing few of the results so printing few more information on the statistics file.
  ;       -- *1) Issue was due to the modification for GrpCtAl2a onwards.
  ...
  ;-- CT file variable. --
  Crs_talk = 'Inv5_Noct_7a_'+ STRN(PSDEff)
  print, Crs_Talk
  ...
  for p =0, nfiles-1 Do Begin
    ...
    For i =0L, nevents-1 do Begin
      ...
      For k = 0,NoAnodes-1 Do Begin
        ...
        ;
        Temp_Arr = Grp_CT_Al_3(New_AnID[k],New_AnNrg[k])
    ...
    Outfile= Strmid(infile, 0,Temp_pos)+Crs_talk+'.Inv3.Ssts.txt'
    ...
    Printf, lun3, 'Before_CT Hard_Dec Hard_after_CT Hard_after_ST Soft'
    for i =0,7 Do begin
