Pro gsim_grp_all_compare, file1, file2, title=title, bin=bin
  ...
  ;==== PLOT Variables
  ;
  PTitle = ' PC Flight vs Simulation  (CT=15%)'
  Xtitle = ' Energy (keV) '
  Ytitle = ' C/s/keV '
  ...
  for i = 0, nbins-2 do begin
    ...
    TotalGamma[i] = tempGam+tempPele+TempPpos+TempPPro+TempSposD+TempSPosU+TempSEleU+TempSEleD+TempSProD+TempSProU
  ...
  Cgplot, Xval2,Flight2, PSYM=3, XRANGE=[Xmin,Xmax],Xstyle=1, /Xlog,/Ylog,  YRAnge=[Ymin,Ymax], YStyle=1,$
    ...
    err_XHigh = Xerr, Err_Xlow=Xerr
    
   ; CgLegend, SymColors=['black','red','ORG4','Gold','GRN4','medium orchid', 'sky blue','gray'],PSyms=[1,1,1,1,1,1,1,1], Location=[0.93,0.9],$
            ;  Titles=['Flight','Gamma','Pri Elec','Pri Pos', 'Pri Prot', 'Sec Prot', 'Sec Elec', 'Sec Posi'],length=0, Tcolors=['black','red','ORG4','Gold','GRN4','medium orchid','sky blue','gray'], /box, charsize=1
            CgLegend, SymColors=['black','red','ORG4','Gold','GRN4'],PSyms=[1,1,1,1,1], Location=[0.93,0.9],$
              Titles=['Flight','Gamma','Pri Elec','Pri Pos', 'Pri Prot'],length=0, Tcolors=['black','red','ORG4','Gold','GRN4'], /box, charsize=1
    CGoplot, Xval2,TotalGamma, PSYM=0, Color='blue',    err_XHigh = Xerr, Err_Xlow=Xerr,Err_Width=er_wid,/Err_Clip;,$
  ;  err_Yhigh = TotalGamma_Err, err_Ylow = TotalGamma_Err , /Err_Clip, Err_Color='blue'
...
Cgerase
  ...
  Cgplot, Xval2,Flight2, PSYM=3, XRANGE=[Xmin,1000],Xstyle=1, /Xlog, /Ylog, YRAnge=[Ymin,Ymax], YStyle=1,$
      title=ptitle, xtitle=xtitle, ytitle=ytitle,$
      err_Yhigh=Flight2_Err, err_Ylow = Flight2_Err, /Err_Clip,Err_Width=er_Wid,Err_Color='black',$
          err_XHigh = Xerr, Err_Xlow=Xerr
        CgLegend, SymColors=['black','red','ORG4','Gold','GRN4'],PSyms=[1,1,1,1,1], Location=[0.79,0.9],$
          Titles=['Flight','Gamma','Pri Elec','Pri Pos', 'Pri Prot'],length=0, Tcolors=['black','red','ORG4','Gold','GRN4'], /box
  ...
  ;    err_Yhigh = Gamma2_Err, err_Ylow = Gamma2_Err , /Err_Clip, Err_Color='red'
    ...
    CGoplot, Xval2,PPosi2, PSYM=3, Color='Gold',    err_XHigh = Xerr, Err_Xlow=Xerr,Err_Width=er_wid,$
      err_Yhigh = PPosi2_Err, err_Ylow = PPosi2_Err , /Err_Clip, Err_Color='Gold'
    CGoplot, Xval2,PProt2, PSYM=3, Color='GRN4',    err_XHigh = Xerr, Err_Xlow=Xerr,Err_Width=er_wid,$
      err_Yhigh = PProt2_Err, err_Ylow = PProt2_Err , /Err_Clip, Err_Color='GRN4'
  ;    CGoplot, XVal2, Sprot, PSYM=3, color='red'
