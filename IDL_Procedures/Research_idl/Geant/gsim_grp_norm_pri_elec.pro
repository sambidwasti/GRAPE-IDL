Pro GSim_GRP_norm_pri_elec, file, title=title, bin=bin, N=N, Type=Type, foldername=foldername, Class = Class, Sep = Sep
  ;
  ...
  ;   print, infile
  openr, Lun222, infile , /Get_Lun
  For i = 0L, nevents-1 do Begin

    readu, Lun222, GSim_Struc_1        ; read one event
  ...
  priele_pc = temp_histogram

  priele_err = scl_factor*sqrt(abs(priele_pc))

  prielectron = scl_factor*priele_pc
  ; print, prielectron
  ...
  openw, lun233, foldername1+title1+'_'+Evt_Text+'_l1_PriElec_com.txt',/Get_Lun
  for i = 0, 999 do begin
    Printf, Lun233, Strn(prielectron[i]), ' ', strn(priele_err[i])
  endfor
  free_lun, lun233
