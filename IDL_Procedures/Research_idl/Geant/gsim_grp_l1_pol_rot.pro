Pro GSim_Grp_l1_Pol_rot, fsearch_String, title=title
  ;
  ; This is based on gsim_grp_l1_hist
  ; This is to extract rotation angle from file name and do the scatter angle histogram for all 91 files.
  ...
  For p = 0, Nfiles-1 Do Begin
    ...
    ; XXX_Deg
    pos0 = StrPos(infile,'deg',0)
    pos1 = StrPos(infile,'_', Pos0-5)
    rtang= Float(strmid(infile, pos1+1,3))
    Print, 'Rot Ang:',rtang
