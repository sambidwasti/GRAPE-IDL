Pro GSim_GRP_l1_process, fsearch_String, title=title, PSDeff=PSDeff, Per=Per
  ...
  ; June 20, 2017
  ;    - Commenting out the AUx hardware threshold to verify few things. 
  ;;
  True =1
  False=0
...
print,'PC Before:', n_elements(gsimdata0[where(gsimdata0.EvClass eq 3)])
    ...
    OutFile = INfile
    StrPut, OutFile, 'psd',OutPos+5
    OutFile = Outfile +'_'+strn(Psdeff)+'_ct_'+ strn(per)+'_lvl1.dat'
