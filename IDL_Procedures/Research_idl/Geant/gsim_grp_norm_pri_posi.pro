Pro GSim_GRP_norm_pri_posi, file, title=title, bin=bin, N=N, Type=Type, foldername=foldername, Class = Class, Sep = Sep
  ;
  ...
  ; help, temp_histPC
  priposi_pc = temp_histogram

  priposi_err = scl_factor*sqrt(abs(priposi_pc))

  pripositron = scl_factor*priposi_pc
  ; print, prielectron
  ...
  openw, lun2, foldername1+title1+'_'+Evt_Text+'_l1_PriPosi_com.txt',/Get_Lun
  for i = 0, 999 do begin
    Printf, Lun2, Strn(pripositron[i]), ' ', strn(priposi_err[i])
  endfor
  free_lun, lun2
