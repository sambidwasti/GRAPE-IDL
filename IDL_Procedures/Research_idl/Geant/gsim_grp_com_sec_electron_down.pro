Pro GSim_GRP_com_sec_electron_down, fsearch_String, title=title, bin=bin, type=type, foldername=foldername, N=N, Class = Class, Sep = Sep
  ...
  For p = 0, Nfiles-1 Do Begin
    ...
    openr, Lun333, infile , /Get_Lun
    For i = 0L, nevents-1 do Begin

      readu, Lun333, GSim_Struc_1        ; read one event
  ...
  if folderflag eq true then begin
   ...
   openw, lun331,foldername1+title1+'_'+Evt_Text+'_l1_SecDownElec_com.txt',/Get_Lun
  for i = 0, 999 do begin
    Printf, Lun331, Strn(hist_tot[i]), ' ', strn(hist_err[i])
