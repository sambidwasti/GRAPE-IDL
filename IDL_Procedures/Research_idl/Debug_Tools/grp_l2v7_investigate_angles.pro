Pro Grp_L2v7_Investigate_Angles, fsearch_String
  ;
  ; Purpose of L2v7 Investigate is to verify the output so that things are consistent.
  ;
  ; This program is mainly to investigate the Grp L2 proceessed angles.
  ;
  ...
  For p = 0, nfiles-1 Do Begin ; open each file
    ...
    If data.Nanodes NE 2 Then goto, Jump_Packet
      Rot_angle = [Rot_Angle, data.RtAng]
      Rot_Status = [Rot_Status, data.RtStat]
      Pv_Angle = [ Pv_Angle, data.PvAng]
      
      
      If Event_data[i].PvAng gt 0 then begin
         
          Print, ' Class = ',data.EvClass
          print
          
          print, ' Pri ID =',data.pa_id
          print, ' Sec ID =',data.sa_id
          print, ' Scat ANgle=', data.Sctang
          
          print
          Print,' Table Rot Angle=',data.RtAng
         Print, ' PvAng=' ,data.PvAng
         print, 'ppvang=', data.ppvang
         print, 'hrang =', data.hrang
         print, 'parang=', data.parang
         print, 'posang=', data.posang
         print
         print, data.sctang-data.rtang
         Print, Data.sctang-data.rtang-data.pvAng
    ...
    Rot_Status= Rot_Status[1:N_Elements(Rot_Status)-1]
    Pv_Ang    = Pv_Angle[1:N_elements(Pv_Ang)-1]
    Help, Pv_Ang
    CgPs_Open, 'Investigate_angle.ps', Font =1, /LandScape
    cgLoadCT, 13
    
    CgPlot, Indgen(N_Elements(Rot_Angle), /LONG), Rot_Angle, Title=' Table Angle Variation',Ytitle=' Table Angle ', XTitle='Event No'
    CgErase
    
    CgPlot, Indgen(N_Elements(Rot_Status), /LONG), Rot_Status, Title=' Rot Motor Status',Ytitle=' Rot Motor Status Value ', XTitle='Event No'
    CgErase
    
    CgPlot, Indgen(N_Elements(Pv_Ang), /LONG), Pv_Ang, Title=' PV Angle',Ytitle=' Angle ', XTitle='Event No'
    CgErase
    
    CgPs_Close

    ;-- Create the PDF File
    Temp_Str = Cur+'/Investigate_angle.ps'
