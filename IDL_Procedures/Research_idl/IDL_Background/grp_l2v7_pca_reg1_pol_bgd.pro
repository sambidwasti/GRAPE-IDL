pro grp_l2v7_pca_reg1_pol_bgd,  pcafile, N= N, LoEner = LoEner , HiEner = HiEner, title=title, fig=fig
  ; This is variation from pca_reg1_pol for focusing on Bgd4. 
  ...
  RegVar1 = AllVar1[0:57]
  RegVar2 = AllVar2[0:57]
  RegVar3 = AllVar3[0:57]
  RegVar4 = AllVar4[0:57]
  RegVar5 = AllVar5[0:57]
  RegVar6 = AllVar6[0:57]
  RegVar7 = AllVar7[0:57]

    
  ; Changing CrabVar1 to BgdVar1
  BgdVar1= AllVar1[58:64]
  BgdVar2= AllVar2[58:64]
  BgdVar3= AllVar3[58:64]
  BgdVar4= AllVar4[58:64]
  BgdVar5= AllVar5[58:64]
  BgdVar6= AllVar6[58:64]
  BgdVar7= AllVar7[58:64]
  ...
  For p = 0, No_e_range-1 Do Begin
    ...
    InStruc = Grp_l2v7_PCA_rate_Gen_1_pol_bgd(fsearch_Str, Emin=Emin , Emax=Emax)
    ...
    RegSwp = InStruc[0:57].Swp
    RegRate= InStruc[0:57].Rate
    RegErr = Instruc[0:57].Err

    ;----
    BgdSwp = Instruc[58:64].Swp
    BgdRate= Instruc[58:64].rate
    BGdErr = Instruc[58:64].err

    ;
    Print, 'Bgd4 Sweep', BGdSwp
    Print, ' Counts', BgdRate
    ...
    Fit_reg_bgd = Reg_Const+ BgdVar1*Regress_result[0]+BgdVar2*Regress_result[1]+BGdVar3*Regress_result[2]+BgdVar4*Regress_result[3]+$
      BGdVar5*Regress_result[4]+BgdVar6*Regress_result[5]+BgdVar7*Regress_result[6]
   
     Reg_DOF= n_elements(RegSwp)-7-1
     Reg_RChi = Reg_Chi/Reg_DOF
      
      Reg_Yfit2_Err2 =  Reg_Sigma[0]*REg_sigma[0]*bgdVar1*bgdVar1+ Reg_Sigma[1]*Reg_sigma[1]*BgdVar2*BGdVar2 + Reg_Sigma[2]*Reg_sigma[2]*BGdVar3*BGdVar3 +$
       Reg_Sigma[3]*Reg_sigma[3]*BgdVar4*BgdVar4 + Reg_Sigma[4]*Reg_sigma[4]*BGdVar5*BgdVar5 + Reg_Sigma[5]*Reg_sigma[5]*BGdVar6*BGdVar6 + Reg_Sigma[6]*Reg_sigma[6]*BgdVar7*BgdVar7
  ...
  ;  PRINT, CHECK_MATH(PRint=1)
    ...
    Print, Chisq
    Yfit2=Result1[0]+BgdVar1*Result1[1]+BgdVar2*Result1[2]+BgdVar3*Result1[3]+BgdVar4*Result1[4]+$
      BgdVar5*Result1[5]+BgdVar6*Result1[6]+BgdVar7*Result1[7]
    ...
    Yfit2_Err2 = Sigma[0]^2+ Sigma[1]*sigma[1]*BgdVar1*BgdVar1+ Sigma[2]*sigma[2]*BgdVar2*BgdVar2 + Sigma[3]*sigma[3]*BgdVar3*BgdVar3 +$
      Sigma[4]*sigma[4]*BGdVar4*BgdVar4 + Sigma[5]*sigma[5]*BgdVar5*BgdVar5 + Sigma[6]*sigma[6]*BgdVar6*BgdVar6 + Sigma[7]*sigma[7]*BGdVar7*BgdVar7
    ...
    For j = 0,N_Elements(BgdSwp)-1 Do Begin
    ...
    ;  CGoPlot, RegSwp, Yfit, Color='Blue'
      CgoPlot, BgdSwp, Yfit2, Color='Firebrick', Err_YHigh=Yfit2_err,Err_YLow=Yfit2_err, Err_Color='Light Salmon',err_width=0, thick=2
...
;      Cgoplot, bgdSwp, Fit_Reg_bgd, Color='Blue',Err_YHigh=Reg_Yfit2_err,Err_YLow=Reg_Yfit2_err, Err_Color='Cornflower Blue',err_width=0, thick=2
    ...
    openw, lun555, 'Bgd_qlook.txt', /get_lun
    ...
    for j = 0, n_elements(Bgdswp)-1 do begin
      printf, lun555
      printf, lun555, Bgdswp[j], yfit2[j]
    endfor
    free_lun, lun555
