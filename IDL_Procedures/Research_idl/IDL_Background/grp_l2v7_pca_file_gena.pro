Pro Grp_l2v7_PCA_file_Gena, fsearch_String, Title= Title, nfiles=nfiles

  ; temporary file before editing of Gen..
  ; THis is the first file generator for the PCA>
  ; Getting this from flight investigate2 .
  ; This was used for the generation of one of the pca files.
  ; This is to streamline for the background estimation through pca.
  ; Not focused on ETYpe or class atm so they are non existent atm.
  ;
  ...
  Printf,lun1, 'Sweep    Time        Dep      Zen     Count    CountErr'
  For i=1,  N_Elements(Sum_Count)-1 Do Begin
    Printf, Lun1, Sum_Sweep[i],Sum_Time[i], Sum_Dep[i], SUm_Zen[i], Sum_Count[i], Sum_Count_Err[i],$
      format = '(I4,1X, F11.2,1X, F8.2,1X,  F8.2,1X, F8.2,1X, F8.2,1X,F8.2,1X, F8.2, 1X,F8.2, 1X, F8.4 )'
  Endfor
  Free_Lun, Lun1
