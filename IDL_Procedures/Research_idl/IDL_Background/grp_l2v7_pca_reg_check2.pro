pro grp_l2v7_pca_reg_check2,  pcafile, N= N
  ; This is to check the output and confirm the regression steps.
  ...
  ReadCol, pcafile, AllVar1, AllVar2, AllVar3, AllVar4, AllVar5, AllVar6,AllVar7,AllVar8,format='D,D,D,D,D,D,D,D'
...
;  X1 = [Transpose(RegVar1), Transpose(RegVar2), Transpose(RegVar3), Transpose(RegVar4),Transpose(RegVar5), Transpose(RegVar6), Transpose(RegVar7)]
  X1 = [Transpose(RegVar1), Transpose(RegVar2), Transpose(RegVar3), Transpose(RegVar4),Transpose(RegVar5), Transpose(RegVar6), Transpose(RegVar7),Transpose(RegVar8)]
  ...
  Yfit2=Con+CrabVar1*Result1[0]+CrabVar2*Result1[1]+CrabVar3*Result1[2]+CrabVar4*Result1[3]+$
    CrabVar5*Result1[4]+CrabVar6*Result1[5]+CrabVar7*Result1[6]+CrabVar8*Result1[7]

  Yfit1a = Con+RegVar1*Result1[0]+RegVar2*Result1[1]+RegVar3*Result1[2]+RegVar4*Result1[3]+$
    RegVar5*Result1[4]+RegVar6*Result1[5]+RegVar7*Result1[6]+RegVar8*Result1[7]

  Print, total(yfit2)
  filename = 'PCA_'+STRN(FIX(EMIN_a))+'_'+STRN(Fix(EMAX_a))+'PC_9var_low'
