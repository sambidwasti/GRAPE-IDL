function Grp_l2v7_PCA_rate_Gen_1_pol_Bgd, fsearch_String, Emin = Emin, Emax = Emax, nfiles=nfiles
  ; 
  ; This is a varaition from PCA_Rate_gen_1_Pol (which is for Crab) for Bgd4. 
  ; And that was a  variation from rate gen 1 for Crab
  ; 
  ; NOTE: This does not affect the pca stage as this funciton is versatile for any pca analysis.
  ;       However, it is easier to debug issue if we focus on each analysis and avoid making 
  ;       any unforseen changes that would effect the other analysis.
  ;
  ; This is to generate a count per sweep input for a specific energy range for the pca.
  ; We need this for polarization.
  ; Note: Changing the counting rate and error to double from float.
  ; Some of the values were hitting the ceiling.
  ...
  For p = 0, nfiles-1 Do Begin ; open each file
    ...
    ;print, evt_Cnt, 'EVT CNT PCA RATE'
    ...
    ;   We are doing time difference 1 so that it is not per second but total for that sweep.
    Time_diff = 1;720.00
